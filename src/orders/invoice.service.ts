  ...
  async generateInvoice(orderId: number, clientId: number): Promise<Buffer> {
    ...
    if (order.status !== OrderStatus.COMPLETED) {
      throw new BadRequestException(
        'Invoice hanya tersedia untuk pesanan yang sudah selesai.',
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        ...
        .text(`Rp ${totalAmount.toLocaleString('id-ID')}`, 460, 290, {
          align: 'right',
          width: 78,
        });
      ...
      doc
        ...
        .text(`Rp ${totalAmount.toLocaleString('id-ID')}`, 460, totalsY + 8, {
          align: 'right',
          width: 78,
        });

      doc
        .text('Admin Fee (komisi platform)', 360, totalsY + 24)
        .text(`Rp ${adminFee.toLocaleString('id-ID')}`, 460, totalsY + 24, {
          align: 'right',
          width: 78,
        });

      doc
        .moveTo(350, totalsY + 42)
      ...
      doc
        ...
        .text(`Rp ${totalAmount.toLocaleString('id-ID')}`, 460, totalsY + 48, {
          align: 'right',
          width: 78,
        });
      ...
      doc
        ...
        .fillColor('#777')
        .text(
          `(Vendor menerima Rp ${netAmount.toLocaleString('id-ID')} setelah potongan komisi)`,
          360,
          totalsY + 65,
        );
      ...
      doc
        ...
        .fillColor('#999')
        .text(
          'Dokumen ini diterbitkan secara otomatis oleh sistem Vendor Marketplace.',
          50,
          730,
          { align: 'center', width: 495 },
        )
        .text(
          'Dapat digunakan sebagai bukti transaksi untuk keperluan LPJ atau administrasi kampus.',
          50,
          742,
          { align: 'center', width: 495 },
        );
