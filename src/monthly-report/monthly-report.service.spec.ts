...
describe('MonthlyReportService', () => {
  ...
  describe('generateReport', () => {
    it('should throw if period format is invalid', async () => {
      await expect(
        service.generateReport({ period: 'invalid' }),
    ...
    it('should throw if report already exists', async () => {
      ...
      await expect(
        service.generateReport({ period: '2023-10' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should generate report successfully', async () => {
      ...
      (prisma.order.aggregate as jest.Mock).mockResolvedValueOnce({
        _sum: { adminFee: new Decimal(100) },
      });
      (prisma.featuredPlacement.aggregate as jest.Mock).mockResolvedValue({
  ...
  describe('updateOperationalCost', () => {
    it('should throw if report not found', async () => {
      (prisma.monthlyReport.findUnique as jest.Mock).mockResolvedValue(null);
      await expect(
        service.updateOperationalCost(1, { operationalCost: 100 }),
    ...
    it('should update cost', async () => {
      ...
      (prisma.monthlyReport.update as jest.Mock).mockResolvedValue({
        ...
        totalGmv: new Decimal(1000),
        grossRevenue: new Decimal(150),
        operationalCost: new Decimal(100),
        netProfit: new Decimal(50),
        cscShare: new Decimal(30),
        cciShare: new Decimal(20),
      });
  ...
  describe('processDividend', () => {
    it('should update dividend shares', async () => {
      ...
      (prisma.monthlyReport.update as jest.Mock).mockResolvedValue({
        id: 1,
        totalGmv: new Decimal(1),
        grossRevenue: new Decimal(1),
        operationalCost: new Decimal(1),
        netProfit: new Decimal(1),
        cscShare: new Decimal(1),
        cciShare: new Decimal(1),
      });

      await service.processDividend(1, {
        cscPercentage: 50,
        cciPercentage: 50,
      });
      expect(prisma.monthlyReport.update).toHaveBeenCalled();
